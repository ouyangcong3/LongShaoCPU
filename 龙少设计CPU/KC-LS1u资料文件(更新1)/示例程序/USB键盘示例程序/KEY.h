unsigned char KEY_XXX(unsigned char keydata) //判断8个字节键盘数据的后7个字节是否有和keydata相同值,有则返回非0
{
   unsigned char i = 0;
...
unsigned char KEY_ift() //取8个字节键盘数据的后7个字节的最后一个键值
{
   unsigned char i = 8;
   while(--i,i != 1 && KEY_DATA[i] == 0);
   return KEY_DATA[i];
}
...
unsigned char KEY_Spacebar() //Spacebar 空格键
{
   return KEY_XXX(0x2c);
}
