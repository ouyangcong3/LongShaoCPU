...
CS_8080 = 0x0a; //"CS2" 片选信号线为低电平, 选中接 "CS2" 的设备
CS_8080 = 0x0b; //"CS3" 片选信号线为低电平, 选中接 "CS3" 的设备
CS_8080 = 0x0c; //"CS4" 片选信号线为低电平, 选中接 "CS4" 的设备
CS_8080 = 0x0d; //"CS5" 片选信号线为低电平, 选中接 "CS5" 的设备
CS_8080 = 0x0e; //"CS6" 片选信号线为低电平, 选中接 "CS6" 的设备
CS_8080 = 0x0f; //"CS7" 片选信号线为低电平, 选中接 "CS7" 的设备
...
把 "CTVGA_C" 的位0置0请求访问 "VRAM0" 的话, 可能会导致之前写入 "VRAM0" 的数据显示不完整;
   ...
   读写显存操作VRAM[]数组,数组偏移为16位数,高8位为Y坐标地址,低8位为X坐标地址,或者使用机器指令操作显存也可以,参看"VGA.h"函数示例;
