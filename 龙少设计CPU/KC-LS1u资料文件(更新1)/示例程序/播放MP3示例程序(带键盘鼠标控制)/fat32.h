...
unsigned char DEVICE_RD_512B(unsigned char device,unsigned long int SQH,unsigned char *buf)  //设备读512字节,device选择设备,SQH为扇区号,buf缓冲区,返回值不为0表示操作成功
{
        ...
        if((device & 0xfe) == 0x00) return TF_SD_RD_512B(device,SQH,buf);
   else return 0;
...
unsigned char DEVICE_WR_512B(unsigned char device,unsigned long int SQH,unsigned char *buf)  //设备写512字节,device选择设备,SQH为扇区号,buf缓冲区,返回值不为0表示操作成功
{
        ...
        if((device & 0xfe) == 0x00) return TF_SD_WR_512B(device,SQH,buf);
   else return 0;
...
//////////////////////////////提取引导扇区地址/////////////////////////////////////////////////////////////
    ...
    if(DEVICE_RD_512B(device,0,buf) == 0) return 0;   //读出0号扇区到缓冲区

///////////////////////提取簇大小、FAT表数量、FAT1表起始扇区号、FAT2表起始扇区号、数据区起始扇区号///////////

    if(DEVICE_RD_512B(device,x = buf[0x01c7]*256 + buf[0x01c6],buf) == 0) return 0;   //读出引导扇区到缓冲区
    ...
    if(buf[0x0052] != 'F' || buf[0x0053] != 'A' || buf[0x0054] != 'T' || buf[0x0055] != '3' || buf[0x0056] != '2') return 0; //不是"FAT32"时,返回0
...
unsigned char FAT32_wjcx_1(unsigned char device,unsigned long int mulu_cuhao,unsigned int *wjlj,unsigned char *return_data) //FAT32文件查询1,device选择设备,mulu_cuhao指定起始目录首簇号,wjlj为文件路径,以0作为路径结束符,return_data为返回的32个字节文件信息数据, 返回值不为0表示操作成功
{
...
unsigned int wjm_c[403]; //记录长文件名,如果是长文件名,文件名格式标志为1
...
wjlj_cd = 0;
while(wjlj_cd < 403 && wjlj[wjlj_cd] != 0) ++wjlj_cd; //取文件路径长度
          ...
          do
          {
               ...
               else
               {
                  ...
                  if(wjm_gsbz == 0)
                  {
                     ...
                     if(wjm_hzm_k == 0 && wjm_hzm_x > 8 || wjm_hzm_k == 1 && wjm_hzm_x > 11) wjm_gsbz = 1; //超出8_3短文件名格式时标志为长文件名
...
unsigned char _culianX;
     ...
     if(_cuhao == 0x0fffffff) return 2; //为簇结束标志,进入处理
...
unsigned char dwjm_dxx_k; //短文件名大小写控制字节,如果短文件名的前8个字节全部为小写,位4置1,后3个字节为小写,位3置1
  ...
  while(wjmcd != 403 && WJM[_wjm_smzz] != 0) //取文件名长度
  {
     if(WJM[_wjm_smzz] < 128) ++_wjm_smzz;else _wjm_smzz += 2;
     ++wjmcd;
  }
...
unsigned char WJFDATA_32B_byte_0CH;
     ...
     FAT32_WJMFL_DWJM_CWJM(WJM,FAT32_ML_SETUP_WJ_WJFDATA_32B,FAT32_ML_SETUP_WJ_CWJM,&WJM_GS,&WJFDATA_32B_byte_0CH,1);
...
unsigned char FAT32_ML_SETUP_WJ_RUN(unsigned char device) //运行目录创建文件,device选择设备,返回值0错误,1继续,2完成,3存储设备空间不足
{

unsigned char returnX;


     if(FAT32_ML_SETUP_WJ_XZ == 0) //扫描空白簇初始化
     {

         FAT32_VOID_cuhao(device,0); //扫描空白簇初始化,扫FAT1
         FAT32_ML_SETUP_WJ_XZ = 1; //切换到运行扫描空白簇
         returnX = 1;

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 1) //运行扫描空白簇
     {

         returnX = FAT32_VOID_cuhao_RUN(device); //运行扫描空白簇,返回值1继续,2找到空白簇,0设备错误,3存储设备满

         if(returnX == 2) //找到空白簇
         {
             FAT32_ML_SETUP_WJ_XZ = 2; //切换到初始化更新文件簇链长度
             returnX = 1;
         }

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 2) //初始化更新文件簇链长度
     {

         FAT32_UP_culian(FAT32_VOID_cuhao_cuhao,FAT32_ML_SETUP_WJ_WJDX,FAT32_ML_SETUP_WJ_LBA_VOID_K); //初始化更新簇链长度
         FAT32_ML_SETUP_WJ_XZ = 3; //切换到运行更新簇链长度
         returnX = 1;

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 3) //运行更新文件簇链长度
     {

         returnX = FAT32_UP_culian_RUN(device); //运行更新簇链长度,device选择设备,返回0错误,1继续,2完成,3存储设备空间不足

         if(returnX == 2) //更新簇链长度完成
         {
             FAT32_ML_SETUP_WJ_XZ = 4; //切换到目录创建文件符初始化
             returnX = 1;
         }

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 4) //目录创建文件符初始化
     {
         ...
         FAT32_ML_SETUP_WJ_XZ = 5; //切换到运行目录创建文件符
         returnX = 1;

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 5) //运行目录创建文件符
     {

         returnX = FAT32_ML_SETUP_WJF_RUN(device); //运行目录创建文件符,device选择设备,返回值0错误,1继续,2完成,3设备空间不足

         if(returnX == 3) //设备空间不足
         {
             FAT32_ML_SETUP_WJ_XZ = 6; //初始化更新文件簇链长度(清除文件簇链)
             returnX = 1;
         }

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 6) //初始化更新文件簇链长度(清除文件簇链)
     {

         FAT32_UP_culian(FAT32_VOID_cuhao_cuhao,0,0); //初始化更新簇链长度
         FAT32_ML_SETUP_WJ_XZ = 7; //切换到运行更新簇链长度
         returnX = 1;

     }
     else if(FAT32_ML_SETUP_WJ_XZ == 7) //运行更新文件簇链长度(清除文件簇链)
     {

         returnX = FAT32_UP_culian_RUN(device); //运行更新簇链长度,device选择设备,返回0错误,1继续,2完成,3存储设备空间不足(创建簇链)
...
unsigned char FAT32_ML_WJCX(unsigned char device,unsigned long int mulu_cuhao,unsigned char *WJM,unsigned char *RETURN_WJFDATA_32B,unsigned int *RETURN_CWJM,unsigned char E5_DELETE_K) //目录下文件查询,device选择设备,mulu_cuhao为目录首簇号,WJM为文件名,0作为结束符,RETURN_WJFDATA_32B为返回的32个字节文件符数据,RETURN_CWJM为返回的长文件名,以0作为结束符,E5_DELETE_K为1读出文件符数据后填E5删除文件符,为0不删除文件符
{
                                ...
                                while(1)
                                {
                                    ...
                                    while(1)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if(E5_DELETE_K == 1) buf[CWJM_ADDR_smzz] = 0xe5;
                                            CWJM_ADDR_smzz += 32;
                                            --CWJM_ADDR_CD;
                                 ...
                                 if(E5_DELETE_K == 1) //进入填E5,删除文件
                                 {
                                     ...
                                     while(1)
                                     {
                                        ...
                                        while(1)
                                        {
                                            ...
                                            if(CWJM_ADDR_CD == 65535) //长度为-1,处理完成
                                            {
                                                if(DEVICE_WR_512B(device,CWJM_ADDR_sqh_,buf) == 0) return 0; //缓冲区存盘
                                                goto out2;
                                            }
...
unsigned char returnX;
    ...
    returnX = FAT32_ML_WJCX(device,mulu_cuhao,WJM,WJFDATA_32B,CWJM,1);
...
unsigned char FAT32_ML_DELETE_WJ_RUN(unsigned char device) //运行目录删除文件,device选择设备,返回值0错误,1继续,2完成
{
...
unsigned char returnX;
    ...
    if(FAT32_ML_DELETE_WJ_XZ == 0) //初始化更新文件簇链长度(清除文件簇链)
    {
         ...
         FAT32_ML_DELETE_WJ_XZ = 1; //切换到运行更新簇链长度

         if(FAT32_ML_DELETE_WJ_cuhao == 0) returnX = 2;else returnX = 1; //簇号为0完成
    }
    else if(FAT32_ML_DELETE_WJ_XZ == 1) //运行更新文件簇链长度(清除文件簇链)
    {
         returnX = FAT32_UP_culian_RUN(device); //运行更新簇链长度,device选择设备,返回0错误,1继续,2完成,3存储设备空间不足(创建簇链)
    }
...
unsigned char js1;
    ...
    returnX = FAT32_ML_WJCX(device,mulu_cuhao,WJM_,FAT32_ML_XGWJM_S_WJFDATA_32B,FAT32_ML_XGWJM_S_CWJM,1); //取出原文件符,并填E5删除原文件符
...
unsigned char FAT32_ML_XGWJM_RUN(unsigned char device)
{

unsigned char returnX;


     if(FAT32_ML_XGWJM_XZ == 0) //目录创建文件符初始化
     {
         ...
         FAT32_ML_XGWJM_XZ = 1; //切换到运行目录创建文件符
         returnX = 1;

     }
     else if(FAT32_ML_XGWJM_XZ == 1) //运行目录创建文件符
     {
         ...
         if(returnX == 3) //设备空间不足
         {
             FAT32_ML_XGWJM_XZ = 2; //切换到目录创建文件符初始化(恢复原文件符)
             returnX = 1;
         }

     }
     else if(FAT32_ML_XGWJM_XZ == 2) //目录创建文件符初始化(恢复原文件符)
     {
         ...
         FAT32_ML_XGWJM_XZ = 3; //切换到运行目录创建文件符
         returnX = 1;

     }
     else if(FAT32_ML_XGWJM_XZ == 3) //运行目录创建文件符(恢复原文件符)
     {
...
unsigned int WJM_smzz;
     ...
     while(1)
     {
          ...
          if(_WJM == 0) //文件路径结束
          {
              ...
              return FAT32_ML_WJCX(device,mulu_cuhao,WJM,RETURN_WJFDATA_32B,RETURN_CWJM,0); //查询此目录文件
          ...
          else if(_WJM == '\') //目录符
          {
              ...
              WJM[WJM_smzz] = 0; //填0结束符
              if(FAT32_ML_WJCX(device,mulu_cuhao,WJM,RETURN_WJFDATA_32B,RETURN_CWJM,0) != 1) return 0; //查询此目录文件
